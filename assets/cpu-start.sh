...
#  fi
  ...
  env PARAM_lorawan-driver_driver-spec="$spec" PARAM_lorawan-driver_customer=$id \
      LOG_USE_STDOUT=1 LOG_STDOUT_PREFIX="$(printf "%04d " $id)" \
      virtcpu virtcpu-run $((4002000000 + id)) $localhost &
done
